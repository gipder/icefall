...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use random clustering for categories instead of IPA-based clustering.",
    )
    """
...
def run(rank, world_size, args):
    ...
    elif params.category_method == "ipa":
    ...
    ksponspeech = None
    aishell = None
    libri_plus_kspon = None
    
    for dataset in params.dataset:
        if dataset == "all":
            logging.info("Using all datasets")
            logging.info("But it only uses Korean and English datasets")
    ...
    if params.use_full_dataset:
        ...
        if aishell is not None:
            ...
            train_cuts_list.append(aishell.train_cuts())
        if libri_plus_kspon is not None:
            logging.info("Using 1/10 cuts for both LibriSpeech and KsponSpeech in LibriPlusKspon")
            train_cuts_list.append(libri_plus_kspon.train_200_cuts())
    ...
    if libri_plus_kspon is not None:
        ...
        valid_cuts_list.append(libri_plus_kspon.valid_cuts())
