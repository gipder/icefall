...
def decode_dataset(
    ...
    time_per_sample = elapsed_time / num_cuts if num_cuts > 0 else 0
    logging.info(
        f"Decoding finished. Total samples: {num_cuts}, "
        f"elapsed time: {elapsed_time:.1f} seconds, time per sample: {time_per_sample:.3f} seconds"
    )
...
def main():
    ...
    params.res_dir = params.exp_dir / f"monitor_{params.decoding_method}"
    ...
    for dataset in params.dataset:
        if dataset == "all":
            ...
            aishell_test_dl = asr_datamodule.test_dataloaders(
                aishell_test_cuts)
            test_sets.extend(["aishell_test"])
            test_dls.extend([aishell_test_dl])            
        else:
            raise ValueError(f"Unsupported dataset: {dataset}")
