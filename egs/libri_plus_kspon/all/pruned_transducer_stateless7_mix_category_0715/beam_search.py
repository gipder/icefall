...
def modified_beam_search(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        # if the shape of logits is (num_hyps, 1, 1, 1, vocab_size), squeeze it three times
        if logits.ndim == 5:
            logits = logits.squeeze(1).squeeze(1).squeeze(1)
        else:
            logits = logits.squeeze(1).squeeze(1) # original code
        # logits = logits.squeeze(1).squeeze(1).squeeze(1)  # (num_hyps, vocab_size)
