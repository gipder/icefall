    ...
    if use_beam_search:
        ...
        with torch.no_grad():
                    ...
                    for n in range(num_nbest):
                        for i in range(feature_lens.size()[0]):
                            ...
                            if hyp_len != encoder_len:
                                ...
                                # case 1. hyp_cache[ids[i]][n] is shorter than encoder_lens
                                if hyp_len < encoder_len:
            ...
            for n in range(num_nbest):
                ...
                pseudo_labels = list()
                #for i in range(len(hyp_tokens)):
                for i in range(len(hyp_tokens_list[n])):
                    tmp_pseudo_label = [ t for t in hyp_tokens_list[n][i] if t != 0 ]
                ...
                for i in range(len(hyp_tokens_list[n])):
                    if hyp_tokens_list[n][i][0] != 0:
                        hyp_tokens_list[n][i][1:] = hyp_tokens_list[n][i][:-1] # shift to right
                        hyp_tokens_list[n][i][0] = 0
                # get max length of hyp_tokens
                max_len = 0
                for i in range(len(hyp_tokens_list[n])):
                    if max_len < len(hyp_tokens_list[n][i]):
                        max_len = len(hyp_tokens_list[n][i])
                alignment = torch.zeros((len(hyp_tokens_list[n]), max_len), dtype=torch.int32)

                # get pseudo_y_sequence
                pseudo_y_sequence = alignment.clone().to(device)
                for i in range(len(hyp_tokens_list[n])):
                    pseudo_y_sequence[i, :len(hyp_tokens_list[n][i])] = torch.tensor(hyp_tokens_list[n][i], dtype=torch.int32)

                # remove duplicated labels in hyp_tokens
                # and convert the sequence to monotonic increasing sequence
                for i in range(len(hyp_tokens_list[n])):
                    mask = (torch.tensor(hyp_tokens_list[n][i], dtype=torch.bool) != 0)
                    alignment[i, :len(hyp_tokens_list[n][i])] = torch.cumsum(mask.int(), dim=-1, dtype=torch.int32)
