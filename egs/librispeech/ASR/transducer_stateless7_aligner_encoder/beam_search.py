...
def aligner_search(
    ...
    """Aligner Greedy search for a single utterance.
    Args:
      model:
        An instance of `Transducer`.
      encoder_out:
...
def aligner_beam_search(
    model: Transducer,
    encoder_out: torch.Tensor,
    beam: int = 4,
    return_timestamps: bool = False,
    use_debiasing: bool = False,
    debiasing_alpha: float = 2.0,
) -> Union[List[int], DecodingResults]:
    """It limits the maximum number of symbols per frame to 1.
    ...
    for t in range(T):
        ...
        ys_log_probs = ys_log_probs.unsqueeze(-1)
        decoder_input = torch.tensor(
        ...
        # debiasing to remove the effect of label smoothing
        if use_debiasing:
        ...
        topk_log_probs, topk_indexes = log_probs.topk(cbeam)
