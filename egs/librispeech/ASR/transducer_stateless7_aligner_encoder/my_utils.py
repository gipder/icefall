...
def create_pseudo_alignment(
        x_len,
        y_len,
        is_even_alignment=False,
        make_zero=False):
    """
    ...
    if is_even_alignment is False:
        base_repeats = torch.ones(padded_y_len, dtype=int)
        base_repeats = base_repeats.to(device=x_len.device)
        remaining_len = x_len - padded_y_len
        padded_y_len = padded_y_len.to(device=x_len.device)
        pvals = torch.ones(padded_y_len, device=x_len.device)/padded_y_len
        additional_repeats = torch.multinomial(
            pvals,
            remaining_len,
            replacement=True)
        additional_repeats_count = torch.bincount(
            additional_repeats, minlength=padded_y_len)
        repeats = base_repeats + additional_repeats_count
    ...
    if make_zero:
        diff = torch.diff(ret, prepend=ret[:1])
        repeated_indices = torch.where(diff == 0)[0]
        ret[repeated_indices] = 0
