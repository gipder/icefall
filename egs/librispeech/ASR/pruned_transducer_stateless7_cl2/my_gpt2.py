...
class GPT2LMScorer(TransformersLMScorer):
    ...
    def _build(self, model_name: str, options: Dict[str, Any]) -> None:
        ...
        # pylint: disable=attribute-defined-outside-init
        tmp_model_name = "/home/sskim/work/transformers/examples/pytorch/language-modeling/model/"
        #tmp_model_name = "./gtp2_trained"
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_name, use_fast=True, add_special_tokens=False, local_files_only=True#, config=tmp_model_name +"/config.json"
        )
        #self.tokenizer = AutoTokenizer(tmp_model_name)
        ...
        self.model = GPT2LMHeadModel.from_pretrained(tmp_model_name)
...
class AutoLMScorer(GPT2LMScorer):
...
def main():
    #scorer = AutoLMScorer.from_pretrained("ggpt2", device='cuda', batch_size=4)
    scorer = AutoLMScorer.from_pretrained("/home/sskim/work/transformers/examples/pytorch/language-modeling/model", device='cuda', batch_size=4)

    sent = "Hello World"
    sent2 = "Hwllo world"
    sent3 = "HHllo world"
    sent4 = "HHHHHHHHH asdfs asdf"
    sent_list = [sent, sent2, sent3, sent4]
    for i in sent_list:
        print(scorer.sentence_score(i, reduce="prod"))
