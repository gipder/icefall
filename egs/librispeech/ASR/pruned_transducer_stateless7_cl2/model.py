...
class Transducer(nn.Module):
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            simple_loss, (px_grad, py_grad), tmp_px, tmp_py = k2.rnnt_loss_smoothed(
                lm=lm.float(),
                am=am.float(),
                symbols=y_padded,
        ...
        py_std = 0
        if py_var:
            alignment = ranges[:, :, 0].unsqueeze(1)
            if d_verbose: print("ranges.shape: " + str(ranges.shape))
            if d_verbose: print("alignment.shape: " + str(alignment.shape))
            #if d_verbose: print("alignment: " + str(alignment))
            pivot = torch.gather(tmp_py, 1, alignment)
            tmp_py_sqrt = torch.sqrt(torch.pow(tmp_py - pivot, 2))
            tmp_std = torch.mean(tmp_py_sqrt, dim=1)
            py_std = torch.mean(tmp_std)
        ...
        if px_var:
            alignment = ranges[:, :, 0].unsqueeze(1)
            #if d_verbose: print("alignment: " + str(alignment))
            pivot = torch.gather(tmp_px, 1, alignment)
            #print(pivot)
            #print(tmp_px)
            #print(tmp_px[:, :, :-1])
            tmp_px_sqrt = torch.sqrt(torch.pow(tmp_px[:, :, :-1] - pivot, 2))
            tmp_px_std = torch.mean(tmp_px_sqrt, dim=1)
            #print(tmp_px_std)
            px_std = torch.nanmean(tmp_px_std)
            #print("px_std: " + str(px_std))
        ...
        return (simple_loss, pruned_loss, py_std, px_std)
