...
from zipformer import Conv2dSubsampling
...
class Conformer(EncoderInterface):
    ...
    def forward(
        ...
        Args:
          x:
            The model input. Its shape is (N, T, feature_dim).
          x_lens:
            A tensor of shape (batch,), which contains the length of each input sequence.
        Returns:
            Tensor: Predictor tensor of dimension (batch_size, input_length, d_model).
            Tensor: A tensor of shape (batch,) which contains the length of each input sequence
        """
        x = self.encoder_embed(x)
        x, pos_emb = self.encoder_pos(x)
        x = x.permute(1, 0, 2)  # (B, T, F) -> (T, B, F)
        mask = encoder_padding_mask(x.size(0), supervisions=None)
        if mask is not None:
        ...
        x = x.permute(1, 0, 2)  # (T, B, F) -> (B, T, F)
        x_lens = torch.div(x_lens-1, 2, rounding_mode='trunc').int()
        x_lens = torch.div(x_lens-1, 2, rounding_mode='trunc').int()
