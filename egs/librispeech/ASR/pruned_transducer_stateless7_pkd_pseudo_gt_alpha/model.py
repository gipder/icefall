...
class Transducer(nn.Module):
    ...
    def __init__(
        ...
        self.alphas_criterion = nn.MSELoss(reduction='mean')
    ...
    def forward(
        ...
        if torch.isnan(pruned_loss).any() or torch.isinf(pruned_loss).any():
            ...
            print(f"{idx=}")
            print(f"{x[0, 0]=}")
            print(f"{encoder_out[0,0]=}")
            print(f"{logits[0, 0]=}")
            print(f"{x_lens=}")
            print(f"{y_lens=}")
            print(f"{ranges[idx[0]]=}")
            print(f"{y_padded[idx[0]]=}")
            print(f"{y_padded[0]=}")
            import sys
            sys.exit(0)
        ...
        if use_alphas:
            ...
            tpx, tpy = k2.get_rnnt_logprobs_pruned(
            ...
            #spy = logits - torch.logsumexp(logits, dim=-1, keepdim=True)
            spx, spy = k2.get_rnnt_logprobs_pruned(
                logits=logits.float(),
                symbols=y_padded,
                ranges=ranges,
                termination_symbol=blank_id,
            ...
            talphas = teacher_model.get_alphas_from_torch(tpx, tpy, boundary=boundary, use_grad=False)
            salphas = self.get_alphas_from_torch(spx, spy, boundary=boundary, use_grad=True)
            print(f"{salphas[0]=}")
            print(f"{alphas[0]=}")
            print(f"{salphas[-1]=}")
            print(f"{alphas[-1]=}")
            def set_inf_beyond_boundary(alphas, boundary):
                for i in range(alphas.shape[0]):
                    t_len = boundary[i, -1]
                    s_len = boundary[i, -2]
                    alphas[i, :, t_len+1:] = -torch.inf
                    alphas[i, s_len+1:, :] = -torch.inf
                return alphas
            talphas = set_inf_beyond_boundary(talphas, boundary)
            salphas = set_inf_beyond_boundary(salphas, boundary)
            salphas += torch.randn_like(salphas) * 1e-5
            #salphas = alphas
            #print(f"{talphas[0]=}")
            #print(f"{salphas[0]=}")
            mask = torch.isfinite(talphas) & torch.isfinite(salphas)
            print(f"{salphas[0, boundary[0 ,-2], boundary[0, -1]]=}")
            print(f"{alphas[0, boundary[0, -2], boundary[0, -1]]=}")
            talphas = talphas[mask]
            salphas = salphas[mask]
            print(f"after masking")
            print(f"{talphas[0]=}")
            print(f"{salphas[0]=}")
            #alphas_loss = self.pkd_criterion(salphas, talphas)
            # mse loss
            a = talphas - salphas
            a2 = torch.abs(a)
            mask = torch.isfinite(a2)
            # counting True
            number = mask.sum(dim=-1).sum(dim=-1) #)torch.sum(mask, dim=-1)
            # set to zero if not finite
            a2[~mask] = -1e16
            # sum all
            print(f"{a2.sum(dim=-1).sum(dim=-1)=}")
            al = a2.sum(dim=-1).sum(dim=-1) / number
            al = torch.mean(al)
            alphas_loss = al #torch.sum(a2, dim=-1) / number
            #print(f"{a2[0]=}")
            #noise = torch.randn_like(talphas) * 0.01
            #alphas_loss = self.alphas_criterion(talphas.float()+noise, alphas.float())
            print(f"{alphas_loss=}")
            """
    ...
    def get_ranges_logits_and_so_on(
        self,
        x: torch.Tensor,
        x_lens: torch.Tensor,
        y: k2.RaggedTensor,
        ...
        if not use_grad:
            logits = logits.detach_()
        masks = None
        ret = (ranges, logits, y_padded, blank_id, boundary)
    ...
    def get_alphas2(
        ...
        #bp = logits - torch.logsumexp(logits, dim=-1).unsqueeze(-1)
        assert px.ndim == 3, px.shape
        B, S, T1 = px.shape
        T = py.shape[-1]
        assert px.shape[-1] in [T, T + 1], (px.shape, T)  # if T, then "modified".
        ...
        # alphas
        my_alphas = torch.empty((B, S + 1, T + 1), dtype=torch.double, device=px.device)
        my_alphas[:, 0, 0] = 0.0
        my_alphas[:, 1:, 0] = -torch.inf
        my_alphas[:, 0, 1:] = -torch.inf

        my_alphas[:, 0, 1:T+1] = torch.cumsum(py[:, 0, :T], dim=-1)
        my_alphas[:, 1:S+1, 0] = torch.cumsum(px[:, :S, 0], dim=-1)
        for s in range(1, S+1):
            for t in range(1, T+1):
                my_alphas[:, s, t] = torch.logaddexp(my_alphas[:, s-1, t] + px[:, s-1, t],
                                                     my_alphas[:, s, t-1] + py[:, s, t-1])
        # betas
        # TODO: optimization
        my_betas = torch.empty((B, S + 1, T + 1), dtype=torch.double, device=px.device)
        my_betas[:,:,:] = -torch.inf
        for b in range(B):
            SS = boundary[b, -2]
            TT = boundary[b, -1]
            my_betas[b, SS, TT] = py[b, SS, TT-1]

        for b in range(B):
            SS = boundary[b, -2]
            TT = boundary[b, -1]
            for t in range(TT-1, 0, -1):
                my_betas[b, SS, t] = my_betas[b, SS, t+1] + py[b, SS, t]

        for b in range(B):
            SS = boundary[b, -2]
            TT = boundary[b, -1]
            for s in range(SS-1, 0, -1):
                my_betas[b, s, TT] = my_betas[b, s+1, TT] + px[b, s, TT]

        for b in range(B):
            SS = boundary[b, -2]
            TT = boundary[b, -1]
            for s in range(SS, 0, -1):
                for t in range(TT, 0, -1):
                    my_betas[b, s-1, t-1] = torch.logaddexp(my_betas[b, s, t-1] + px[b, s-1, t-1],
                                                        my_betas[b, s-1, t] + py[b, s-1, t-1])
        ...
        my_alphas[:, 1:, 0] = -torch.inf
        my_alphas[:, 0, 1:] = -torch.inf
        # initialize the alphas for the first py
        for b in range(B):
            for t in range(1, T):
                my_alphas[b, 0, t] = my_alphas[b, 0, t-1] + py[b, 0, t-1]

        for b in range(B):
            for s in range(1, S+1):
                my_alphas[b, s, 0] = my_alphas[b, s-1, 0] + px[b, s-1, 0]

        for b in range(B):
            for s in range(1, S+1):
                for t in range(1, T+1):
                    my_alphas[b, s, t] = torch.logaddexp(my_alphas[b, s-1, t] + px[b, s-1, t],
                                                         my_alphas[b, s, t-1] + py[b, s, t-1])
        """
        # backward
        """
        my_betas = torch.empty((B, S + 1, T + 1), dtype=torch.double, device=px.device)
        ...
        myT = boundary[0, -1]
        myS = boundary[0, -2]
        print(f"{py[0, myS, myT-1]=}")
        print(f"{my_alphas[0, myS, myT]=}")
        print(f"{my_alphas[0, myS, myT-1]=}")

        print(f"{alphas[0, myS, myT]=}")
        print(f"{alphas[0, myS, myT-1]=}")

        print(f"{my_betas[0]=}")
        print(f"{my_betas[0, myS, myT]=}")
        print(f"{my_betas[0, 0, 0]=}")

        print(f"{my_alphas[0, 0, 1]=}")
        print(f"{my_alphas[0, 1, 0]=}")
        print(f"{my_betas[0, 0, 1]=}")
        print(f"{my_betas[0, 1, 0]=}")
        print(f"{my_alphas[0, myS, myT] + my_betas[0, myS, myT]=}")
        g1 = my_alphas[0, 0, 1] + my_betas[0, 0, 1]
        v1 = torch.exp(my_alphas[0, 0, 1] + my_betas[0, 0, 1])
        v2 = torch.exp(my_alphas[0, 1, 0] + my_betas[0, 1, 0])
        print(f"{torch.log(v1 + v2)=}")
        #print(f"{boundary[0]=}")
        #print(f"{my_alphas[0, 74, 330]=}")
        #print(f"{alphas[0, 74, 330]=}")
        #print(f"{my_alphas[1, boundary[1, -2], boundary[1, -1]]=}")
        #print(f"{alphas[1, boundary[1, -2], boundary[1, -1]]=}")

        #print(f"{px[0]=}")
        #print(f"{py[0]=}")
        #print(f"{alphas[0]=}")
        #print(f"{alphas[0, 1, 1]=}")
        #print(f"{torch.logaddexp(alphas[0, 0, 1]+px[0, 0,1], alphas[0, 1, 0]+py[0, 1, 0])=}")
        #print(f"{alphas[0, 0, 1]=}")
        #print(f"{px[0, 0, 1]=}")
        #print(f"{alphas[0, 1, 0]=}")
        #print(f"{py[0, 1, 0]=}")
        #print(f"{torch.log(torch.exp(alphas[0,0,1]+px[0,0,1])+torch.exp(alphas[0,1,0]+py[0,1,0]))=}")
        #print(f"{alphas[0,0,1]+px[0,0,1]=}")
        #print(f"{torch.exp(alphas[0,0,1]+px[0,0,1])=}")
        #print(f"{alphas[0,1,0]+py[0,1,0]=}")
        #print(f"{torch.exp(alphas[0,1,0]+py[0,1,0])=}")
        #print(f"{alphas[0].shape=}")
        return alphas

    def get_alphas_from_torch(
        self,
        px: torch.Tensor,
        py: torch.Tensor,
        ...
        # alphas
        my_alphas = torch.empty((B, S + 1, T + 1), dtype=torch.float, device=px.device)
        my_alphas[:, 0, 0] = 1e-6
        my_alphas[:, 1:, 0] = -torch.inf
        my_alphas[:, 0, 1:] = -torch.inf
        ...
        for s in range(1, S+1):
            for t in range(1, T+1):
                my_alphas[:, s, t] = torch.logaddexp(my_alphas[:, s-1, t] + px[:, s-1, t],
                                                     my_alphas[:, s, t-1] + py[:, s, t-1])
        if torch.isnan(my_alphas).any():
            print(f"{torch.where(torch.isnan(my_alphas))[0]=}")
        if not use_grad:
            my_alphas = my_alphas.detach_()

        return my_alphas

    def get_betas_from_torch(
        self,
        px: torch.Tensor,
        py: torch.Tensor,
        boundary: torch.Tensor = None,
        ...
        # betas
        # TODO: optimization
        my_betas = torch.empty((B, S + 1, T + 1), dtype=torch.float, device=px.device)
        my_betas[:,:,:] = -torch.inf
        for b in range(B):
            SS = boundary[b, -2]
            TT = boundary[b, -1]
...
if __name__ == "__main__":
    ...
    for idx, batch in enumerate(test_clean_dl):
        ...
        print(f"{feature_lens.shape=}")
        print(f"{y.shape=}")
        ranges, logits, y_padded, blank_id, boundary = model.get_ranges_logits_and_so_on(x=feature,
                                                                                         x_lens=feature_lens,
                                                                                         y=y, prune_range=5,
                                                                                         use_grad=False)
        px, py = k2.get_rnnt_logprobs_pruned(
        ...
        teacher_alphas = model.get_alphas_from_torch(px, py, boundary, use_grad=False)
        teacher_betas = model.get_betas_from_torch(px, py, boundary, use_grad=False)

        sranges, slogits, sy_padded, sblank_id, sboundary = smodel.get_ranges_logits_and_so_on(x=feature,
                                                                                         x_lens=feature_lens,
                                                                                         y=y, prune_range=5,
                                                                                         use_grad=True)
        spx, spy = k2.get_rnnt_logprobs_pruned(
            ...
            symbols=sy_padded,
            ranges=sranges,
            termination_symbol=sblank_id,
            boundary=sboundary,
            rnnt_type="regular",
        )

        student_alphas = smodel.get_alphas_from_torch(spx, spy, boundary, use_grad=True)
        student_betas = smodel.get_betas_from_torch(spx, spy, boundary, use_grad=True)
        mask1 = torch.isfinite(teacher_alphas)
        mask2 = torch.isfinite(teacher_betas)
        smask1 = torch.isfinite(student_alphas)
        smask2 = torch.isfinite(student_betas)

        print(f"{teacher_alphas=}")
        print(f"{student_alphas=}")
        print(f"{torch.equal(mask1, mask2)=}")
        print(f"{torch.equal(smask1, smask2)=}")
        print(f"{torch.equal(mask1, smask1)=}")
        print(f"{torch.equal(mask2, smask2)=}")
