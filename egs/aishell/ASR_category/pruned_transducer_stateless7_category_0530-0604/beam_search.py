...
def greedy_search_batch(
    ...
    l_hyps = [[-1] * (context_size - 1) + [blank_id] for _ in range(N)]
    ...
    decoder_category_input = torch.tensor(
        l_hyps,
        device=device,
        dtype=torch.int64,
    )  # (N, context_size)

    decoder_out = model.decoder(decoder_input, decoder_category_input, need_pad=False)
    decoder_out = model.joiner.decoder_proj(decoder_out)
    ...
    for t, batch_size in enumerate(batch_size_list):
        ...
        logits, category_logits = model.joiner(
            current_encoder_out, decoder_out.unsqueeze(1), project_input=False
        )
        # logits'shape (batch_size, 1, 1, vocab_size)
        logits = logits.squeeze(1).squeeze(1)  # (batch_size, vocab_size)
        category_logits = category_logits.squeeze(1).squeeze(1) # (batch_size, category_size)
        assert logits.ndim == 2, logits.shape
        assert category_logits.ndim == 2, category_logits.shape

        if blank_penalty != 0:
            logits[:, 0] -= blank_penalty

        y = logits.argmax(dim=1).tolist()
        l = category_logits.argmax(dim=1).tolist()
        ...
        if emitted:
            ...
            decoder_category_input = [l[-context_size:] for l in l_hyps[:batch_size]]
            decoder_category_input = torch.tensor(
                decoder_category_input,
                device=device,
                dtype=torch.int64,
            )
            decoder_out = model.decoder(decoder_input, decoder_category_input, need_pad=False)
