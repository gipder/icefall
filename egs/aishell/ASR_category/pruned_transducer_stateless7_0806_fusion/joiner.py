...
class Joiner(nn.Module):
    def __init__(
        ...
        self.w_low1_t = nn.Linear(joiner_dim, joiner_dim)
        self.w_low2_t = nn.Linear(joiner_dim, joiner_dim)
        self.w_proj = nn.Linear(joiner_dim, joiner_dim)
    ...
    def forward(
        ...
        gate = torch.sigmoid(
            self.w_gate1(encoder_out) + self.w_gate2(decoder_out)
        )
        ...
        h_hat = self.w_proj(
            torch.tanh(self.w_low1_t(encoder_out)) * torch.tanh(self.w_low2_t(h_gate))
        )

        logit = self.output_linear(
            torch.tanh(
                h_hat + self.w_joint1(encoder_out) + self.w_joint2(decoder_out)
            )
        )
