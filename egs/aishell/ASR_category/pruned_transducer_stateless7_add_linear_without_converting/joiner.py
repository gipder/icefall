...
class Joiner(nn.Module):
    def __init__(
        ...
        self.additional_linear = nn.Linear(
            vocab_size, self.num_category
        )
        self.output_linear = nn.Linear(self.num_category, vocab_size)
    ...
    def forward(
        ...
        #import sys
        #sys.exit(0)
        logit = self.middle_linear(torch.tanh(logit))
        logit = self.additional_linear(logit)
