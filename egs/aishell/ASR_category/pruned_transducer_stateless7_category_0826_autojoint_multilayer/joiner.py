...
class Joiner(nn.Module):
    def __init__(
        ...
        self.num_category = len(num_category_list)
        ...
        for i in range(self.num_category):
            if i == 0:
                ...
                bottom = num_category_list[i-1]
            up = num_category_list[i]
            self.enc_layers.append(nn.Linear(bottom, up))
        # decoder
        for i in range(self.num_category-1, -1, -1):
            if i == 0:
                up = vocab_size
            else:
                up = num_category_list[i-1]
            bottom = num_category_list[i]
            self.dec_layers.append(nn.Linear(bottom, up))
    ...
    def forward(
        ...
        category_logit_list = list()
        for i, layer in enumerate(self.enc_layers):
            logit_org = layer(logit_org)
        category_logit_list.append(logit_org)
        for i, layer in enumerate(self.dec_layers):
            logit_org = layer(logit_org)
            category_logit_list.append(logit_org)
        #category_logit = self.category_linear(logit_org)
        #logit = self.output_linear(category_logit)
        logit = category_logit_list.pop()
        category_logit_list.reverse()

        return logit, category_logit_list
