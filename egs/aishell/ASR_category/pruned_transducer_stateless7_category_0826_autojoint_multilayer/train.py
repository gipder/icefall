...
from my_utils import map_to_table
from my_utils import cluster_tokens
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=[50],
        help="The list of category layers [first, second, ...].",
    )
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        ...
        vocab_size=params.vocab_size,
        num_category_list=params.num_category,
    )
    return joiner
...
def compute_loss(
    ...
    batch: dict,
    is_training: bool,
    token_to_cluster_table_list: List[Optional[Tensor]] = None,
    ...
    l_list = list()
    for i in range(len(token_to_cluster_table_list)):
        l = list()
        for c in y.tolist():
            c = torch.tensor(c, dtype=torch.long, device=device)
            c_mapped = token_to_cluster_table_list[i][c].clone()
            c_mapped = c_mapped.to(device)

            l.append(c_mapped.tolist())
        l = k2.RaggedTensor(l).to(device)
        l_list.append(l)
    """
    l = map_to_table(
            y,
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, category_loss_list = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
            l_list=l_list,
        ...
        category_loss = sum(category_loss_list) / len(category_loss_list)
        loss = simple_loss_scale * simple_loss + pruned_loss_scale * ((1-alpha) * pruned_loss + alpha * category_loss)
...
def compute_validation_loss(
    ...
    for batch_idx, batch in enumerate(valid_dl):
        loss, loss_info = compute_loss(
            ...
            batch=batch,
            is_training=False,
            token_to_cluster_table_list=token_to_cluster_table_list,
        )
...
def run(rank, world_size, args):
    ...
    # About to cluster the lexicon
    logging.info("Clustering the lexicon")

    token_to_cluster_table_list = list()
    token_dict = {}
    import os
    if not os.path.exists(params.lang_dir / "tokens.txt"):
        logging.info(
            f"Lexicon tokens file {params.lang_dir / 'tokens.txt'} doesn't exist."
        )
    else:
        ...
        for i in range(len(params.num_category)):
            token_cluster = cluster_tokens(token_dict, params.num_category[i]-1)
            ...
            token_to_cluster_table_list.append(token_to_cluster_table)
        logging.info(f"Token to cluster table_list is ready, "
                     f"size: {len(token_to_cluster_table_list[i])=}")
