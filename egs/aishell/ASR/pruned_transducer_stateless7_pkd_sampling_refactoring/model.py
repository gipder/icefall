    ...
    def forward(
        ...
        if use_pkd:
            ...
            if use_efficient:
                ...
                ranges_dict = dict()
                print(f"{student_ranges=}")
                print(f"{teacher_ranges=}")
                ranges_dict["student"] = student_ranges.clone()
                ranges_dict["teacher"] = teacher_ranges.clone()
...
def get_efficient(
    ...
    if is_pruned:
        student_ranges = ranges_dict["student"]
        teacher_ranges = ranges_dict["teacher"]
        print(f"{student_ranges=}")
        print(f"{teacher_ranges=}")
        ranges_indices = student_ranges.view(student_ranges.shape[0], -1)
        student_result = tensor_y_padded[torch.arange(student_ranges.shape[0]).view(-1, 1), ranges_indices]
        student_indices = student_result.view(student_ranges.shape[0], student_ranges.shape[1], -1)
        ranges_indices = teacher_ranges.view(teacher_ranges.shape[0], -1)
        teacher_result = tensor_y_padded[torch.arange(teacher_ranges.shape[0]).view(-1, 1), ranges_indices]
        teacher_indices = teacher_result.view(teacher_ranges.shape[0], teacher_ranges.shape[1], -1)

        student_indices = student_indices.unsqueeze(-1)
        teacher_indices = teacher_indices.unsqueeze(-1)
        py_logits = torch.gather(logits, -1, index=student_indices)
        teacher_py_logits = torch.gather(teacher_logits, -1, index=teacher_indices)
    else:
