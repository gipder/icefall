...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use Monte Carlo CE loss"
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use a soft target when doing Monte Carlo CE loss"
    )

    parser.add_argument(
        "--use-only-simple-loss",
        type=str2bool,
        default=False,
        help="Whether to use only simple loss"
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use Monte Carlo sampling"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Determine how many samples to take in Monte Carlo sampling"
    )
...
def compute_loss(
    ...
    use_monte_carlo_ce = params.use_monte_carlo_ce
    use_only_simple_loss = params.use_only_simple_loss
    use_mc_sampling = params.use_mc_sampling

    with torch.set_grad_enabled(is_training):
        simple_loss = torch.tensor(0.0, device=device)
        pruned_loss = torch.tensor(0.0, device=device)
        aligner_encoder_loss = torch.tensor(0.0, device=device)
        monte_carlo_ce_loss = torch.tensor(0.0, device=device)
        ...
        ret = model(
            ...
            use_monte_carlo_ce=use_monte_carlo_ce,
            use_soft_target=params.use_soft_target,
        ...
        if use_monte_carlo_ce:
            monte_carlo_ce_loss = ret["monte_carlo_ce_loss"]
            rnnt_loss = monte_carlo_ce_loss
        elif use_mc_sampling:
            mc_sampling_loss = ret["mc_sampling_loss"]
            rnnt_loss = mc_sampling_loss
        elif use_aligner_encoder:
            aligner_encoder_loss = ret["aligner_encoder_loss"]
            rnnt_loss = aligner_encoder_loss
        elif use_only_simple_loss:
            pass
        else:
            pruned_loss = ret["pruned_loss"]
            rnnt_loss = pruned_loss
    ...
    if use_monte_carlo_ce:
        info["monte_carlo_ce_loss"] = monte_carlo_ce_loss.detach().cpu().item()
    elif use_mc_sampling:
        info["mc_sampling_loss"] = mc_sampling_loss.detach().cpu().item()
    elif use_aligner_encoder:
        info["aligner_encoder_loss"] = aligner_encoder_loss.detach().cpu().item()
    elif use_only_simple_loss:
        pass
    else:
...
def scan_pessimistic_batches_for_oom(
    ...
    for criterion, cuts in batches.items():
        ...
        print(f"{batch['supervisions']['cut'][-1]=}")
        print(f"{graph_compiler.texts_to_ids(batch['supervisions']['text'])[-1]=}")
        #import sys
        #sys.exit(0)
        device = model.device if isinstance(model, DDP) else next(model.parameters()).device
        ...
        encoder_out, encoder_out_lens = model_clone.encoder(x=feature, x_lens=feature_lens)
        from beam_search import (
            modified_beam_search,
            modified_beam_search_for_kd,
            Hypothesis,
        ...
        print(f"{hyp_tokens.hyps[-1]=}")
        print(f"{hyp_tokens.timestamps[-1]=}")
        try:
            with torch.cuda.amp.autocast(enabled=params.use_fp16):
                loss, _ = compute_loss(
                    params=params,
