...
def modified_beam_search(
    ...
    finalized_B = []
    import pickle
    dump = open("dump.pkl", "wb")
    logit_out = torch.tensor([], device=device)
    for (t, batch_size) in enumerate(batch_size_list):
        start = offset
        end = offset + batch_size
        current_encoder_out = encoder_out.data[start:end]
        current_encoder_out = current_encoder_out.unsqueeze(1).unsqueeze(1)
        # current_encoder_out's shape is (batch_size, 1, 1, encoder_out_dim)
        print(f"Batch_size {current_encoder_out.shape=}")
        offset = end
        ...
        hyps_shape = get_hyps_shape(B).to(device)
        print(f"{hyps_shape.row_ids(1)=}")
        print(f"{hyps_shape.row_ids(1).shape=}")
        ...
        print(f"{current_encoder_out.shape=}")
        logits = model.joiner(
            current_encoder_out,
            decoder_out,
            project_input=False,
        ...
        logit_out = torch.cat((logit_out[0], probs), dim=0)
        log_probs.add_(ys_log_probs)
    ...
    if return_topk:
        ...
        #print(f"{len(B)=}")
        topk_hyps = list()
        topk_best_orig_hyps = [b.topk(k=3, length_norm=True) for b in B]
        topk_best_list = list()
        for a in topk_best_orig_hyps:
            for h in a:
                topk_best_list.append(h.ys[2:])
