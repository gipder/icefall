...
def find_mono_alignment_path(logits: torch.Tensor,
                             x_lens: torch.Tensor,
                             y: torch.Tensor,
                             y_lens: torch.Tensor,
                             blank: int = 0):
...
def find_mono_alignment_optimized(logits: torch.Tensor,
                                  x_lens: torch.Tensor,
                                  y: torch.Tensor,
                                  y_lens: torch.Tensor,
                                  blank: int = 0):
...
def shift_alignment_right(alignment, shift=1, pad_value=0, ignore_value=-100):
    ...
    for i in range(shifted.size(0)):
        ...
        step = shift
        end_idx = len(valid_values) - 1
        length = len(valid_values)
        while step > 0:
            if end_idx < 1:
                break
            if valid_values[end_idx] == pad_value:
                valid_values = torch.roll(valid_values[:length], 1)
                shifted[i, :length] = valid_values
                step -= 1
            else:
                length -= 1
                end_idx -= 1
