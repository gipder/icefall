...
def greedy_search_independent_aligner(
    model: Transducer,
    encoder_out: torch.Tensor,
    max_sym_per_frame: int,
    return_timestamps: bool = False,
    ...
    while t < T and sym_per_utt < max_sym_per_utt:
        ...
        current_encoder_out = encoder_out[:, t:t+1, :].unsqueeze(2)
        # fmt: on
        logits, aligner_logits = model.joiner(
            current_encoder_out, decoder_out.unsqueeze(1), project_input=False
        )
        # logits is (1, 1, 1, vocab_size)

        y = aligner_logits.argmax().item()
        if y != blank_id:
            h = logits.argmax().item()
            hyp.append(h)
            timestamp.append(t)
            decoder_input = torch.tensor([hyp[-context_size:]], device=device).reshape(
                1, context_size
...
def greedy_search_batch_independent_aligner(
    model: Transducer,
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    return_timestamps: bool = False,
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        aligner_logits = aligner_logits.squeeze(1).squeeze(1)  # (batch_size, vocab_size)
        logits = logits.squeeze(1).squeeze(1)  # (batch_size, vocab_size)
        assert aligner_logits.ndim == 2, aligner_logits.shape
        assert logits.ndim == 2, logits.shape
        y = aligner_logits.argmax(dim=1).tolist()
        emitted = False
        for i, v in enumerate(y):
            if v != blank_id:
                u = logits[i].argmax().item()
                if u == blank_id or u == unk_id:
                    u = logits[i].topk(2).indices[1].item()
                hyps[i].append(u)
...
def modified_beam_search_for_kd(
    model: Transducer,
    x: torch.Tensor,
    x_lens: torch.Tensor,
    beam: int = 4,
    ...
    for a in topk_best_orig_hyps:
        for h in a:
            # I have no idea but two "0" are added at the beginning of h.ys
            # I found out that the reason is that the first two tokens are context_size
            topk_best_list.append(h.ys[context_size:])
    #for i in range(len(topk_best_list)):
    #    print(f"{i}: {len(topk_best_list[unsorted_indices[i]])=}")
