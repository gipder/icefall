...
from icefall.utils import AttributeDict, MetricsTracker, setup_logger, str2bool, make_pad_mask
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt2",
        help="GPT2 LLM names from Huggingface, gpt2, gpt-xl, etc",
    )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        #check if the batch size is too long
        
        is_too_long = False
        for i in range(sentence_lengths.size(0)):
            if sentence_lengths[i] > params.max_sent_len:
                sentence_lengths[i] = params.max_sent_len
                logging.warning(f"{x[i].size(-1)} is more than {params.max_sent_len}")                
                is_too_long = True

        if is_too_long:
            x = x[:, :params.max_sent_len]
            y = y[:, :params.max_sent_len]
